...
if st.session_state.df_editor is not None:
    ...
    invalid_rows = [
        ...
        if (err := validate_row(row.to_dict()))
    ]
    if invalid_rows:
    ...
    col_config = {
        col_subir:    st.column_config.CheckboxColumn(col_subir, help="Marcar para subir a Jobber", default=True),
        col_uploaded: st.column_config.CheckboxColumn(col_uploaded, disabled=True),
    ...
    pending_rows      = edited[(edited[col_subir] == True) & (edited[col_uploaded] == False)]
    jobber_connected  = storage.has_tokens()

    if jobber_connected and len(pending_rows) > 0:
        st.markdown("---")
        if st.button(f"{t('btn_upload_jobber')} ({len(pending_rows)})", type="primary", use_container_width=True):
...
if st.session_state.pop("trigger_upload", False):
    ...
    def get_or_find_client(name: str) -> str:
        ...
        if cached:
            return cached
        res = client.execute(CREATE_CLIENT_MUTATION, {"input": {"companyName": name, "isCompany": True}})
        errors = res["data"]["clientCreate"]["userErrors"]
        if errors:
            raise Exception(f"Error creando cliente '{name}': {errors[0]['message']}")
        new_id = res["data"]["clientCreate"]["client"]["id"]
    ...
    def get_or_create_property(client_id: str, address_str: str) -> str:
        ...
        res_c = client.execute(CREATE_PROPERTY_MUTATION, {"clientId": client_id, "input": {"properties": [prop_input]}})
    ...
    for i, (idx, row) in enumerate(pending.iterrows()):
        ...
        try:
            ...
            results.append({"order": title, "ok": True, "number": job_data["jobNumber"], "url": job_data["jobberWebUri"], "error": ""})
            st.session_state.df_editor.at[idx, col_uploaded] = True
            st.session_state.df_editor.at[idx, col_subir]    = False

        except Exception as e:
            results.append({"order": title, "ok": False, "number": "", "url": "", "error": str(e)})
...
if st.session_state.upload_report:
    ...
    st.dataframe(
        report_df,
        column_config={"Link": st.column_config.LinkColumn("Link", display_text="Abrir en Jobber")},
        use_container_width=True,
        hide_index=True,
    )
    ...
    st.download_button(
        ...
        file_name="reporte_jobber_ea.csv",
        mime="text/csv",
    )
    ...
    if failed and st.session_state.df_editor is not None:
        if st.button(t("btn_retry_failed")):
            ...
            mask = st.session_state.df_editor["Job title Final"].isin(failed_titles)
            st.session_state.df_editor.loc[mask, col_subir]    = True
